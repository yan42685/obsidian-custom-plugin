...
export class InputParticleEffect {
	...
	private lastSpawnTime = 0; // 记录【任何动作】的时间（包括脚本）
	private lastEffectTime = 0; // 记录【真正喷发】的时间（仅限手动）
	...
	public spawn(view: EditorView): void {
		const now = Date.now();
		const diff = now - this.lastSpawnTime;

		// 更新时间戳必须放在最前面，这样才能捕捉到“连发”
		this.lastSpawnTime = now;

		// A. 脚本判定：如果间隔太短，说明是自动化序列
		// 持续更新时间戳会让后续所有 15ms 内的动作全部失效
		if (diff < 15) {
			return;
		}

		// B. 人类节流：如果离上次喷发（diff）小于 50ms，说明按键太快
		// 这里我们可以用一个专门的变量 lastEffectTime 来记录真正喷发的时间
